 ...
 * @author Michael Lang
 */
public abstract class FollowersListCallbacks extends AbstractUserListCallbacks {
    public FollowersListCallbacks(BasePresenter presenter, String login,
            Context context) {
    ...
    public Loader<TaskResult<LoadedPage<User>>> onCreateLoader(int id, Bundle args) {
        ...
        PageIterator<User> iterator = userService.pageFollowers(mLogin, PAGE_ITERATOR_SIZE);
        return new PageIteratorLoader<>(mContext, iterator);
    }
}
