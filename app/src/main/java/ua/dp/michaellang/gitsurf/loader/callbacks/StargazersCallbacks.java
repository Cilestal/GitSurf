 ...
 * @author Michael Lang
 */
public abstract class StargazersCallbacks extends LoaderCallbacks<LoadedPage<User>> {
    private final Context mContext;
    private final RepositoryId mRepositoryId;

    public StargazersCallbacks(BasePresenter presenter, Context context, String owner, String repoName) {
        super(presenter);
        mContext = context;
    ...
    public Loader<TaskResult<LoadedPage<User>>> onCreateLoader(int id, Bundle args) {
        ...
        PageIterator<User> pi
                = service.pageStargazers(mRepositoryId, Constants.PAGE_ITERATOR_SIZE);
        return new PageIteratorLoader<>(mContext, pi);
    }
}
